...
def carregar_contexto_cliente(perfil_path, produtos_path, historico_csv_path, transacoes_csv_path):
    try:
        ...
        # Processamento das Transações para o contexto
...
# Sidebar para inserir a chave de API do Gemini
with st.sidebar:
    st.header("Configurações")
    api_key = "AIzaSyCp3dkYvtG34QR8IwC9UOevR3MnOK47Lck" #st.text_input("Insira sua API Key do Google Gemini:", type="password")
    st.markdown("[Obtenha sua chave no Google AI Studio](https://aistudio.google.com/)")

# Carrega o contexto dos arquivos
...
if api_key:
    ...
    # Inicializar o modelo com o contexto injetado na instrução de sistema (Gemini 1.5 Flash é ótimo para agilidade)
    model = genai.GenerativeModel(
        model_name="gemini-3-flash-preview",
        system_instruction=contexto_sistema
    )
    
    # Inicializar o histórico do chat na sessão do Streamlit
    if "chat_session" not in st.session_state:
        st.session_state.chat_session = model.start_chat(history=[])
        
    # Mostrar o histórico de mensagens na tela
    ...
    # Campo de input de texto para o cliente conversar com o agente
    if prompt := acao if acao else st.chat_input("Digite sua dúvida (ex: Como estão meus gastos este mês? Onde devo investir?)"):
